...
export function SubscriptionControl({ accessToken }: { accessToken?: string }) {
  ...
  const summary = subscription.status === 'canceled'
    ? 'Renovação cancelada.'
    : setupPending
    ...
    <section className="my-5 rounded-xl border border-koma-border p-4 text-sm">
      ...
      {setupPending && (
        <p className="mb-3 text-xs text-koma-muted">
          Enquanto você configura dados, horários e cardápio, a recorrência permanece pausada e nenhum dia do trial é consumido.
        </p>
      )}
