    ...
    <div className="orders-workspace w-full space-y-4 pb-8 text-left animate-fade-in">
      ...
      <section className="flex flex-col gap-3 rounded-[22px] border border-koma-border bg-koma-panel/60 p-3.5 sm:flex-row sm:items-center sm:p-4">
        <div className="relative w-full flex-1">
          ...
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar categoria…" className="h-11 w-full rounded-xl border border-koma-border bg-koma-input pl-10 pr-10 text-[11px] text-white outline-none placeholder:text-zinc-500 focus:border-emerald-400/30" />
      ...
      {filteredCategories.length === 0 ? (
        <div className="flex min-h-64 flex-col items-center justify-center rounded-[22px] border border-dashed border-koma-border bg-koma-panel/60 px-6 text-center">
          ...
          <p className="mt-1 text-[10px] text-zinc-500">Ajuste a busca ou crie uma nova categoria.</p>
        </div>
      ) : (
              ...
              <article key={category.id} className="group flex min-h-[160px] flex-col justify-between rounded-[22px] border border-koma-border bg-koma-panel/60 p-4 shadow-[0_18px_50px_rgba(0,0,0,.16)] transition-colors hover:border-zinc-700">
                <div>
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className="max-w-[60%] truncate rounded-full border border-koma-border bg-koma-input px-2 py-1 font-mono text-[8px] text-zinc-400">{category.id}</span>
                  </div>
                  <h2 className="mt-4 text-sm font-bold text-zinc-100">{category.nome}</h2>
                  <p className="mt-1 text-[10px] leading-relaxed text-zinc-400">{meta.description}</p>
                </div>
                <div className="mt-4 flex items-center justify-between border-t border-koma-border pt-3">
                  ...
                  <div className="flex items-center gap-1">
                    <button type="button" onClick={() => { setEditingCategory(category); setModalOpen(true); }} className="inline-flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[9px] font-bold text-zinc-400 transition-colors hover:bg-white/[0.05] hover:text-white cursor-pointer"><Edit3 size={12} /> Editar</button>
                    <button type="button" onClick={() => { setDeletingCategory(category); setDeleteModalOpen(true); }} className="rounded-lg p-2 text-zinc-500 transition-colors hover:bg-rose-500/10 hover:text-rose-300 cursor-pointer" title="Excluir categoria" aria-label={`Excluir ${category.nome}`}><Trash2 size={13} /></button>
                  </div>
                </div>
              </article>
            );
