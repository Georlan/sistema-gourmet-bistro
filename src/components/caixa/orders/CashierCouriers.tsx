        ...
        <div className={"grid grid-cols-1 lg:grid-cols-3 gap-5 animate-fade-in text-left"}>
          <div className={"lg:col-span-2 bg-koma-card/60 border border-koma-border rounded-3xl p-5 space-y-5 flex flex-col overflow-hidden"}>
            ...
            <div className={"space-y-3 flex-1 overflow-y-auto"}>
              ...
              {deliveryOrders.filter((o) => o.status === 'producao' || o.status === 'analise').length === 0 ? (
                <div className={"py-8 text-center text-koma-muted text-xs italic bg-koma-panel/20 border border-koma-border/40 rounded-2xl"}>
                ...
                <div className="space-y-3">
                  {deliveryOrders
                    ...
                    .map((order) => {
                      ...
                      return (
                        <div key={order.id} className={"p-4 bg-koma-panel border border-koma-border rounded-2xl flex flex-col sm:flex-row justify-between gap-3 text-xs"}>
                          <div className={"space-y-1.5 flex-1"}>
                            <div className={"flex items-center gap-2"}>
                              <span className={"font-bold text-koma-foreground text-[11px]"}>Pedido {order.id}</span>
                            ...
                            <span className={"text-koma-subtle text-[10px] block leading-relaxed"}>{order.endereco}</span>
                            <span className={"text-[9px] text-koma-muted block font-mono"}>Itens: {order.itens}</span>
                          </div>
              ...
              <span className={"text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider block pt-4"}>
              ...
              {deliveryOrders.filter((o) => o.status === 'pronto').length === 0 ? (
                ...
                <div className="space-y-3">
                  {deliveryOrders.filter((o) => o.status === 'pronto').map((order) => (
                    <div key={order.id} className={"p-4 bg-koma-panel/40 border border-koma-border/40 rounded-2xl flex flex-col sm:flex-row justify-between gap-3 text-xs"}>
                      <div className={"space-y-1 flex-1"}>
                        <div className={"flex items-center gap-2"}>
                          <span className={"font-bold text-koma-foreground text-[11px]"}>Pedido {order.id}</span>
                          <span className={"bg-emerald-500/10 text-emerald-400 text-[8px] font-bold px-1.5 py-0.5 rounded border border-emerald-500/20 uppercase tracking-wider"}>Em Trânsito</span>
                        </div>
                        <span className={"text-koma-secondary font-bold block"}>{order.cliente} • {order.telefone}</span>
                        <span className={"text-koma-subtle text-[10px] block leading-relaxed"}>{order.endereco}</span>
                      </div>
                      <div className={"flex flex-col sm:items-end justify-between gap-2 shrink-0"}>
                        <span className={"font-mono font-bold text-emerald-400 text-[11px]"}>R$ {order.total.toFixed(2)}</span>
                        <button type="button" onClick={() => handleFinalizarPedido(order.id)} className={"py-1.5 px-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-[10px] uppercase tracking-wider transition-colors cursor-pointer"}>
                          Concluir Entrega
                        </button>
                      </div>
                    </div>
          ...
          <div className={"bg-koma-card/60 border border-koma-border rounded-3xl p-5 space-y-4 flex flex-col justify-between overflow-hidden"}>
            <div className={"space-y-4 flex-1 flex flex-col overflow-hidden"}>
              <div className={"border-b border-koma-border pb-3 shrink-0"}>
                <span className={"font-serif font-bold text-koma-secondary block text-sm"}>Fretistas Cadastrados</span>
                <span className={"text-[9px] text-koma-muted block"}>Lista de motoboys e entregadores de plantão.</span>
              </div>

              <div className={"flex-1 overflow-y-auto space-y-2.5"}>
                {motoboysLoadState === 'loading' ? (
                  <span className={"text-xs text-koma-muted italic"}>Sincronizando fretistas...</span>
                ) : motoboysLoadState === 'error' ? (
                  <span className={"text-xs text-amber-600 dark:text-amber-300 italic"}>
                    Não foi possível confirmar a lista de fretistas.
                  </span>
                ) : motoboys.length === 0 ? (
                  <span className={"text-xs text-koma-muted italic"}>Nenhum fretista cadastrado.</span>
                ) : (
                  motoboys.map((m) => (
            ...
            >
              <span className={"text-[10px] font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider block"}>Novo Fretista</span>
              ...
              <button type="submit" disabled={motoboysLoadState !== 'loaded'} className={"w-full py-2 bg-emerald-600 hover:bg-[#9d2b3c] disabled:opacity-50 text-white font-bold rounded-xl text-[10px] uppercase tracking-wider transition-colors cursor-pointer"}>
                Adicionar Fretista
              </button>
            </form>
          </div>
