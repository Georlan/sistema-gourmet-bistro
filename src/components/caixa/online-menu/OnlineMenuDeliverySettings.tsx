...
function persistedPayload(config: DeliveryConfig) {
  ...
  return {
    ...
    tipo_taxa_entrega: neighborhoods.length > 0 ? 'bairro' : 'fixa',
...
export function OnlineMenuDeliverySettings({ apiBaseUrl, authHeaders }: Props) {
  ...
  const areasCount = payload.tabela_taxas_bairros.length;
  ...
  const save = async () => {
    ...
    try {
      ...
      setConfig(next);
      setSavedSnapshot(JSON.stringify(persistedPayload(next)));
      setFeedback({ type: 'success', text: 'Entrega e áreas atualizadas na regra canônica.' });
    } catch (error) {
  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <OperationalBanner
        ...
        title="Entrega"
        accent="sem regra duplicada"
        description="Estas regras são as mesmas usadas pelo servidor para validar pedido mínimo, frete grátis e taxa por bairro."
        metrics={[
      ...
      <section className="rounded-2xl border border-koma-border bg-koma-panel p-4 sm:p-5">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-koma-foreground">Disponibilidade do delivery</h3>
              <p className="mt-1 max-w-2xl text-[10px] leading-relaxed text-koma-muted">Desligar aqui bloqueia novos pedidos de entrega no servidor sem afetar retirada.</p>
            </div>
          </div>
          <button
            type="button"
      ...
      <section className="rounded-2xl border border-koma-border bg-koma-panel p-4 sm:p-5">
        <div className="mb-4">
          <h3 className="text-sm font-black text-koma-foreground">Condições de entrega</h3>
          <p className="mt-1 text-[10px] leading-relaxed text-koma-muted">Valores iguais a zero desativam a respectiva condição.</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-2">
          <label>
            ...
            <input
              ...
              placeholder="0,00"
            />
            <span className="mt-1 block text-[9px] text-koma-muted">Abaixo desse subtotal o servidor recusa o delivery.</span>
          </label>
          <label>
            ...
            <input
              ...
              placeholder="0,00"
            />
            <span className="mt-1 block text-[9px] text-koma-muted">Ao atingir o valor, a taxa calculada pelo servidor é zerada.</span>
          </label>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-koma-border bg-koma-panel p-4 sm:p-5">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-koma-foreground">Áreas por bairro</h3>
              <p className="mt-1 max-w-2xl text-[10px] leading-relaxed text-koma-muted">Quando o bairro informado pelo cliente coincide com esta tabela, o servidor aplica a taxa cadastrada. Sem bairros, vale a regra padrão de entrega.</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="space-y-2.5">
          ...
          {config.tabela_taxas_bairros.length === 0 && (
            <div className="rounded-xl border border-dashed border-koma-border p-5 text-center text-[10px] text-koma-muted">Nenhuma área específica cadastrada. O servidor continuará usando a política padrão de entrega.</div>
          )}
        </div>
      </section>
      ...
      <div className="sticky bottom-3 z-20 flex flex-col gap-2 rounded-2xl border border-koma-border bg-koma-panel/95 p-3 shadow-xl backdrop-blur sm:flex-row sm:items-center sm:justify-end">
        {feedback ? (
          ...
          <span className="mr-auto text-[10px] font-semibold text-koma-muted">{hasUnsavedChanges ? 'Há alterações que ainda não foram salvas.' : 'Tudo salvo na configuração operacional.'}</span>
        )}
        <button
          type="button"
