  ...
  return (
    <div className={clsx('grid', 'grid-cols-1', 'lg:grid-cols-3', 'gap-5', 'text-left', 'animate-fade-in')}>
      ...
      <div className={clsx('lg:col-span-2', 'bg-[#121214]/60', 'border', 'border-koma-border', 'rounded-3xl', 'p-5', 'flex', 'flex-col', 'overflow-hidden', 'h-[72vh]')}>
        ...
        <div className={clsx('flex-1', 'bg-koma-page', 'border', 'border-koma-border', 'rounded-2xl', 'p-4', 'flex', 'flex-col', 'justify-between', 'space-y-4')}>
          <textarea
            readOnly
            value={chatbotMessages.map(msg => `[${msg.sender === 'user' ? 'CLIENTE' : 'IA'} - ${msg.timestamp}]: ${msg.text}`).join('\n') + (isBotTyping ? '\n[IA - Digitando...]' : '')}
            className={clsx('w-full', 'flex-1', 'p-3', 'bg-koma-page', 'border', 'border-koma-border', 'rounded-xl', 'text-emerald-500', 'font-mono', 'text-[9px]', 'leading-relaxed', 'focus:outline-none', 'resize-none', 'overflow-y-auto')}
            ref={(el) => {
              if (el) el.scrollTop = el.scrollHeight;
            }}
          />

          <form onSubmit={handleSendChatbotMessage} className={clsx('flex', 'gap-2', 'pt-2', 'border-t', 'border-koma-border', 'shrink-0')}>
            <input
              type="text"
              ...
              className={clsx('flex-1', 'px-4', 'py-2', 'bg-koma-card', 'border', 'border-koma-border', 'rounded-xl', 'focus:outline-none', 'focus:border-[#10b981]', 'text-koma-foreground', 'disabled:opacity-50', 'text-[10px]')}
            />
            <button
              type="submit"
              disabled={!aiBotActive}
        ...
        <div className={clsx('bg-[#121214]/60', 'border', 'border-koma-border', 'rounded-3xl', 'p-5', 'space-y-3', 'text-left')}>
          ...
          <div className="space-y-2">
            {supportChats.length === 0 ? (
              <span className={clsx('text-[10px]', 'text-koma-muted', 'italic', 'block', 'text-center', 'py-5')}>Nenhum chamado pendente</span>
            ) : (
              supportChats.map(chat => (
                <div key={chat.id} className={clsx('bg-koma-card', 'p-3', 'rounded-xl', 'border', 'border-koma-border', 'space-y-2', 'text-left')}>
                  <div className={clsx('flex', 'justify-between', 'items-center', 'text-[10px]')}>
                    <strong className={clsx('text-koma-foreground', 'block', 'truncate', 'w-32', 'font-bold')}>{chat.cliente}</strong>
                    <span className={clsx('text-[8px]', 'uppercase', 'tracking-wider', 'font-mono', 'font-bold', 'text-rose-500')}>{chat.canal}</span>
                  </div>
                  <p className={clsx('text-[10px]', 'text-koma-subtle', 'line-clamp-2', 'leading-relaxed')}>{chat.ultimaMsg}</p>
                  <button
                    onClick={() => {
        ...
        <div className={clsx('bg-[#121214]/60', 'border', 'border-koma-border', 'rounded-3xl', 'p-5', 'space-y-3', 'text-left')}>
          ...
          <div className={clsx('space-y-2.5', 'max-h-[30vh]', 'overflow-y-auto', 'pr-1')}>
            {customerFeedbacks.map(fb => (
              <div key={fb.id} className={clsx('bg-koma-card', 'p-2.5', 'rounded-xl', 'border', 'border-koma-border', 'space-y-1.5', 'text-left')}>
                <div className={clsx('flex', 'justify-between', 'items-center')}>
                  <strong className={clsx('text-koma-foreground', 'block', 'text-[10px]', 'font-bold')}>{fb.cliente}</strong>
                ...
                <p className={clsx('text-[10px]', 'text-koma-subtle', 'italic', 'leading-relaxed')}>"{fb.comentario}"</p>
              </div>
            ))}
          </div>
        </div>
