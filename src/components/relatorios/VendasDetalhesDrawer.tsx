      ...
      <div className={clsx('bg-koma-panel', 'border-l', 'border-koma-border', 'w-full', 'max-w-2xl', 'h-full', 'flex', 'flex-col', 'p-6', 'space-y-4', 'text-left')}>
        ...
        <div className="flex justify-between items-center border-b border-koma-border pb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-serif font-bold text-base text-koma-foreground">Detalhamento de Vendas</h3>
          ...
          <button
            type="button"
            onClick={onClose}
            className="p-1.5 hover:bg-koma-raised rounded-full text-koma-subtle hover:text-koma-foreground transition-colors cursor-pointer"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto border border-koma-border rounded-2xl">
          {isLoading ? (
            <div className="p-8 text-center text-koma-subtle text-xs animate-pulse">
              Carregando detalhamento de vendas...
            </div>
          ) : filteredVendas.length === 0 ? (
            <div className="p-8 text-center text-koma-subtle text-xs">
              ...
              <tbody className="divide-y divide-koma-border">
                {filteredVendas.map((v) => (
                  <tr key={v.id} className="hover:bg-koma-raised/50 transition-colors">
                    <td className="p-3 font-mono text-koma-muted">
                    ...
                    <td className="p-3 text-koma-muted">{v.operador}</td>
