...
import { recoveryToken } from './passwordRecoveryToken';
...
export default function PasswordResetPage() {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(typeof data.detail === 'string' ? data.detail : 'Não foi possível alterar a senha.');
  ...
  return <main className="flex min-h-dvh items-center justify-center bg-koma-page p-4 text-koma-foreground">
    <section className="w-full max-w-sm space-y-4 rounded-2xl border border-koma-border bg-koma-card p-6">
      ...
      {!recoveryToken ? <p>Abra o link enviado ao seu e-mail. Se expirou, solicite outro na tela em que você faz login.</p> : !done && <form onSubmit={submit} className="space-y-4">
