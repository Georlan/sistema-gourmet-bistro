...
export default function SuperAdminWhitelabel({
  ...
  return (
    <div className="space-y-6" id="superadmin-whitelabel-panel">
      {/* Upper Status Banner */}
      <div className="bg-koma-card border border-[#1e293b]/40 p-5 rounded flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
      ...
      <div className="bg-koma-card border border-[#1e293b]/40 p-5 rounded flex flex-col md:flex-row items-center gap-4">
        <div className="w-full md:w-1/3">
          <label className="block text-xs font-mono text-koma-subtle mb-1.5 uppercase tracking-wider font-bold">
        ...
        {selectedTenantId && (
          <div className="flex-1 bg-koma-page border border-[#1e293b]/20 px-4 py-3 rounded text-[11px] font-mono text-koma-subtle flex items-center justify-between">
            <div>
              <span className="text-koma-muted">DOMÍNIO ATUAL: </span>
              <span className="text-koma-foreground font-bold underline hover:text-[#00b894] cursor-pointer">
      ...
      {!selectedTenantId ? (
        <div className="bg-koma-card border border-[#1e293b]/20 p-12 rounded-lg text-center font-mono">
          <Sliders className="w-12 h-12 text-slate-600 mx-auto mb-4 animate-pulse" />
          <h4 className="text-koma-subtle font-bold text-sm">Selecione um Inquilino no painel acima</h4>
        ...
        <div className="bg-koma-card border border-[#1e293b]/20 p-16 rounded-lg text-center font-mono">
          <RefreshCw className="w-8 h-8 animate-spin mx-auto mb-3 text-[#00b894]" />
          <span className="text-xs text-koma-subtle">Puxando dados do cache distribuído e tabelas PostgreSQL...</span>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6" id="whitelabel-layout-grid">
          ...
          <form onSubmit={handleSave} className="xl:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Seção A: Identidade */}
              <div className="bg-koma-card border border-[#1e293b]/40 p-5 rounded flex flex-col space-y-4" id="section-identity">
                <div className="border-b border-[#1e293b]/40 pb-2 flex items-center justify-between">
                  <h4 className="text-xs font-mono text-koma-foreground font-bold flex items-center gap-2">
                    <Globe className="w-4 h-4 text-[#00b894]" />
                    SEÇÃO A: IDENTIDADE WHITELABEL
                  </h4>
                  <span className="text-[8px] text-koma-muted font-mono">ID_DYNAMICS</span>
                </div>

                <div>
                  <label className="block text-[10px] font-mono text-koma-subtle mb-1 uppercase">Subtítulo do Cardápio</label>
                  <input
                    type="text"
                    className="w-full bg-black/60 border border-[#1e293b]/40 rounded px-3 py-2 text-xs font-mono text-koma-foreground focus:outline-none focus:border-[#00b894]"
                ...
                <div>
                  <label className="block text-[10px] font-mono text-koma-subtle mb-1 uppercase">Slug da URL (Único)</label>
                  <input
                    type="text"
                    className="w-full bg-black/60 border border-[#1e293b]/40 rounded px-3 py-2 text-xs font-mono text-koma-foreground focus:outline-none focus:border-[#00b894]"
                    ...
                    required
                  />
                  <span className="text-[9px] text-koma-muted font-mono mt-1 block">
              ...
              <div className="bg-koma-card border border-[#1e293b]/40 p-5 rounded flex flex-col space-y-4" id="section-geoloc">
                <div className="border-b border-[#1e293b]/40 pb-2 flex items-center justify-between">
                  <h4 className="text-xs font-mono text-koma-foreground font-bold flex items-center gap-2">
                    <MapPin className="w-4 h-4 text-[#00b894]" />
                    SEÇÃO B: GEOLOCALIZAÇÃO
                  </h4>
                  <span className="text-[8px] text-koma-muted font-mono">LAT_LON_MAPS</span>
                </div>

                {/* Google Maps URL with Extraction capability */}
                <div>
                  <label className="block text-[10px] font-mono text-koma-subtle mb-1 uppercase">URL do Google Maps do Estabelecimento</label>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      className="flex-1 bg-black/60 border border-[#1e293b]/40 rounded px-3 py-2 text-xs font-mono text-koma-foreground focus:outline-none focus:border-[#00b894]"
                    ...
                    <button
                      ...
                      className="bg-koma-page border border-[#1e293b]/40 hover:border-slate-700 hover:text-[#00b894] px-3.5 rounded text-[11px] font-mono font-bold text-koma-subtle transition-all cursor-pointer disabled:opacity-40"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[10px] font-mono text-koma-subtle mb-1 uppercase">Latitude</label>
                    <input
                      type="number"
                      step="any"
                      className="w-full bg-black/60 border border-[#1e293b]/40 rounded px-3 py-2 text-xs font-mono text-koma-foreground focus:outline-none focus:border-[#00b894]"
                  ...
                  <div>
                    <label className="block text-[10px] font-mono text-koma-subtle mb-1 uppercase">Longitude</label>
                    <input
                      type="number"
                      step="any"
                      className="w-full bg-black/60 border border-[#1e293b]/40 rounded px-3 py-2 text-xs font-mono text-koma-foreground focus:outline-none focus:border-[#00b894]"
                ...
                <div className="pt-3 border-t border-[#1e293b]/20">
                  <label className="block text-[10px] font-mono text-koma-subtle mb-1 uppercase flex items-center gap-1">
                  ...
                  <div className="flex gap-2">
                    <input
                      type="text"
                      className="flex-1 bg-black/60 border border-[#1e293b]/40 rounded px-3 py-1.5 text-xs font-mono text-koma-foreground focus:outline-none focus:border-[#00b894]"
              ...
              <div className="bg-koma-card border border-[#1e293b]/40 p-5 rounded flex flex-col space-y-4" id="section-socials">
                <div className="border-b border-[#1e293b]/40 pb-2 flex items-center justify-between">
                  <h4 className="text-xs font-mono text-koma-foreground font-bold flex items-center gap-2">
                    <Share2 className="w-4 h-4 text-[#00b894]" />
                    SEÇÃO C: REDES SOCIAIS (API DE ENGAJAMENTO)
                  </h4>
                  <span className="text-[8px] text-koma-muted font-mono">COMPILED_SOCIALS_JSON</span>
                </div>

                <div>
                  <label className="block text-[10px] font-mono text-koma-subtle mb-1 uppercase flex items-center gap-1.5">
              ...
              <div className="bg-koma-card border border-[#1e293b]/40 p-5 rounded flex flex-col space-y-4" id="section-payments">
                <div className="border-b border-[#1e293b]/40 pb-2 flex items-center justify-between">
                  <h4 className="text-xs font-mono text-koma-foreground font-bold flex items-center gap-2">
                    <CreditCard className="w-4 h-4 text-[#00b894]" />
                    SEÇÃO D: FORMAS DE PAGAMENTO ACEITAS (CHECKOUT)
                  </h4>
                  <span className="text-[8px] text-koma-muted font-mono">GATEWAY_STRATEGY</span>
                </div>
                ...
                <span className="text-[9px] text-koma-muted font-mono italic">
          ...
          <div className="xl:col-span-1 flex flex-col items-center">
            ...
            <div className="bg-koma-card border border-[#1e293b]/40 p-5 rounded flex flex-col items-center w-full h-full justify-between" id="live-preview-box">
              
              <div className="border-b border-[#1e293b]/40 pb-3 flex items-center justify-between w-full mb-4">
                <div>
                  <h3 className="text-xs font-mono text-koma-foreground font-bold flex items-center gap-2">
                    <Smartphone className="w-4 h-4 text-[#00b894]" />
                    PREVIEW DO CARDÁPIO DIGITAL (MOCK)
                  </h3>
                  <p className="text-[9px] text-koma-muted font-mono mt-0.5 uppercase">Visualização de alterações em tempo real</p>
                </div>
              ...
              <div className="relative w-full max-w-[280px] h-[480px] bg-koma-page rounded-[36px] border-4 border-koma-border p-2.5 shadow-[0_15px_30px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden">
                
                {/* Smartphone Speaker Notch */}
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-24 h-3 bg-koma-raised rounded-b-lg z-20 flex items-center justify-center">
                  <div className="w-6 h-0.5 bg-koma-card rounded-full"></div>
                </div>

                {/* Smartphone Screen Internal */}
                <div className="flex-1 bg-zinc-950 rounded-[26px] overflow-y-auto overflow-x-hidden relative flex flex-col text-koma-foreground pt-4 select-none pb-2 scrollbar-none" id="phone-screen-frame">
                  ...
                  <div className="h-24 bg-gradient-to-br from-[#00b894]/20 via-slate-950 to-zinc-950 relative p-3 flex flex-col justify-end border-b border-zinc-900/40">
                    <div className="absolute top-1.5 right-2.5 bg-black/80 px-1.5 py-0.5 rounded text-[7px] text-koma-subtle font-mono flex items-center gap-1 border border-slate-900">
                      <span>{slug ? `${slug}.koma.com` : "site.koma.com"}</span>
                    </div>
                    <h4 className="text-xs font-bold text-koma-foreground drop-shadow-md tracking-tight">
                      {activeTenantName}
                    </h4>
                  ...
                  <div className="p-3 space-y-3.5 flex-1 flex flex-col justify-between">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-1.5">
                        <p className="text-[8px] font-mono font-bold text-koma-muted uppercase tracking-wider">Produtos em Destaque</p>
                    ...
                    <div className="pt-2 border-t border-zinc-900 space-y-1.5">
                      <p className="text-[7px] font-mono text-koma-muted uppercase font-bold text-center">Formas de Pagamento</p>
                      ...
                      <div className="flex flex-wrap justify-center gap-1">
                        {Object.keys(formasPagamento).filter(key => formasPagamento[key]).length === 0 ? (
                          ...
                          Object.keys(formasPagamento).map(method => formasPagamento[method] && (
                            <span key={method} className="text-[7px] font-mono bg-koma-card text-koma-secondary border border-koma-border px-1 py-0.5 rounded flex items-center gap-0.5">
