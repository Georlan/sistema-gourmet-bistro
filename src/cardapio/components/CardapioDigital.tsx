...
export default function CardapioDigital({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center overflow-y-auto bg-black/75 p-0 sm:items-center sm:p-4 animate-fade-in"
      id="cardapio-checkout-overlay"
    >
      <div
        className="relative flex max-h-[94vh] w-full max-w-2xl flex-col overflow-hidden rounded-t-[30px] border border-koma-border bg-koma-panel text-koma-foreground shadow-2xl sm:rounded-[30px] animate-scale-up"
        id="checkout-card"
      >
        {!createdOrder && (
          <header className="shrink-0 border-b border-koma-border px-5 py-4 sm:px-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.15em] text-emerald-500">
                  <ShoppingBag className="h-3.5 w-3.5" /> Etapa final
                </div>
                <h2 className="mt-1.5 font-display text-lg font-black tracking-tight text-koma-foreground">Revise e confirme</h2>
                <p className="mt-1 text-[10px] text-koma-muted">Nada mais será pedido depois desta tela.</p>
              </div>
              <button
                type="button"
                disabled={isSubmitting}
                onClick={() => !isSubmitting && onClose()}
                className="flex h-11 w-11 min-h-[44px] min-w-[44px] items-center justify-center rounded-full text-koma-muted transition hover:bg-koma-raised hover:text-koma-foreground disabled:cursor-wait disabled:opacity-35"
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-[9px] font-bold">
              {["Sacola", deliveryMethod === "delivery" ? "Entrega" : "Retirada", "Revisão"].map((label, index) => (
                <div
                  key={label}
                  className={index < 2
                    ? "flex items-center justify-center gap-1.5 rounded-lg border border-emerald-500/20 bg-emerald-500/[0.07] px-2 py-2 text-emerald-500"
                    : "flex items-center justify-center gap-1.5 rounded-lg border border-emerald-500/40 bg-emerald-500/15 px-2 py-2 text-koma-foreground"}
                >
                  {index < 2 ? <Check className="h-3 w-3" /> : <span className="grid h-4 w-4 place-items-center rounded-full bg-emerald-500 text-[8px] text-white">3</span>}
                  {label}
                </div>
              ))}
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 no-scrollbar">
          {createdOrder ? (
            <div className="flex flex-col items-center justify-center py-7 text-center sm:py-10 animate-scale-up">
              <div className="grid h-16 w-16 place-items-center rounded-2xl border border-emerald-500/30 bg-emerald-500/15 text-emerald-500">
                <CheckCircle2 className="h-9 w-9" />
              </div>
              <span className="mt-5 text-[9px] font-black uppercase tracking-[0.16em] text-emerald-500">Pedido recebido</span>
              <h3 className="mt-1.5 font-display text-2xl font-black tracking-tight text-koma-foreground">
              ...
              <div className="mt-5 grid w-full max-w-md gap-2 text-left sm:grid-cols-2">
                <div className="rounded-2xl border border-koma-border bg-koma-card p-3.5">
                  <span className="text-[9px] font-black uppercase tracking-wider text-koma-muted">Modalidade</span>
                  <p className="mt-1 text-[11px] font-semibold leading-relaxed text-koma-foreground">
                    {deliveryMethod === "delivery" ? `Entrega · ${address}` : "Retirada no balcão"}
                  </p>
                </div>
                <div className="rounded-2xl border border-koma-border bg-koma-card p-3.5">
                  <span className="text-[9px] font-black uppercase tracking-wider text-koma-muted">Total</span>
                  <p className="mt-1 text-base font-black text-emerald-500">{formatPrice(createdOrder.total ?? estimatedTotal)}</p>
                </div>
              </div>
              ...
              <div className="mt-6 w-full max-w-sm space-y-2">
                ...
                {activeBrand.phone && (
                  <button
                    type="button"
                    onClick={() => {
                      const itensStr = cart.map((item) => `${item.quantity}x ${item.product.name}`).join(", ");
                    ...
                    className="flex h-11 w-full items-center justify-center gap-2 rounded-xl border border-koma-border bg-koma-card px-4 text-[10px] font-bold text-koma-secondary transition hover:border-emerald-500/30 hover:text-emerald-500"
                  >
                    <MessageCircle className="h-4 w-4" /> Falar com o restaurante
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="space-y-5">
              <div className="grid gap-2 sm:grid-cols-2">
                <div className="rounded-2xl border border-koma-border bg-koma-card p-3.5">
                  <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-wider text-koma-muted">
                    <UserRound className="h-3.5 w-3.5 text-emerald-500" /> Pedido em nome de
                  </div>
                  <p className="mt-2 text-xs font-black text-koma-foreground">{customerName}</p>
                  <p className="mt-0.5 text-[10px] text-koma-muted">{customerPhone}</p>
                </div>
                <div className="rounded-2xl border border-koma-border bg-koma-card p-3.5">
                  <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-wider text-koma-muted">
                    <MapPin className="h-3.5 w-3.5 text-emerald-500" /> {deliveryMethod === "delivery" ? "Entrega" : "Retirada"}
                  </div>
                  <p className="mt-2 text-[11px] font-semibold leading-relaxed text-koma-foreground">
                    {deliveryMethod === "delivery" ? address : activeBrand.address || "Retirada no balcão do restaurante"}
                  </p>
                </div>
              </div>
              ...
              <section>
                <div className="mb-2.5 flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={onClose}
                    className="inline-flex items-center gap-1 text-[9px] font-bold text-emerald-500 hover:text-emerald-400"
                  >
                    <ArrowLeft className="h-3 w-3" /> Editar sacola
                  </button>
                </div>
                <div className="space-y-2">
                  {cart.map((item) => {
                    ...
                    return (
                      <div key={item.id} className="flex items-start justify-between gap-3 rounded-xl border border-koma-border bg-koma-card p-3 text-xs">
                        <div className="min-w-0">
                          <span className="block font-black text-koma-foreground">{item.quantity}x {item.product.name}</span>
                          {optionNames.length > 0 && (
                            <span className="mt-0.5 block truncate text-[10px] text-koma-muted">{optionNames.join(", ")}</span>
                          )}
                          {item.notes && (
                            <span className="mt-1 block text-[10px] leading-relaxed text-amber-500">Obs.: {item.notes}</span>
                          )}
                        </div>
                        <span className="shrink-0 font-black text-koma-foreground">{formatPrice(unitPrice * item.quantity)}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <section className="rounded-2xl border border-koma-border bg-koma-card p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="text-xs font-black text-koma-foreground">Pagamento direto ao restaurante</h3>
                    <p className="mt-1 text-[10px] leading-relaxed text-koma-muted">
                      Não há cobrança automática nesta tela. Você paga usando uma das formas aceitas abaixo.
                    </p>
                    {paymentMethods.length > 0 ? (
                      ...
                      <p className="mt-2 text-[9px] font-semibold text-amber-500">O restaurante ainda não informou as formas aceitas no cardápio.</p>
                    )}
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-1.5 border-t border-koma-border pt-4 text-xs">
                ...
                <div className="flex justify-between border-t border-koma-border pt-2 text-sm font-black text-koma-foreground">
                  ...
                  <span className="text-base text-emerald-500">{formatPrice(estimatedTotal)}</span>
                </div>
                <p className="pt-1 text-[9px] leading-relaxed text-koma-subtle">O total definitivo vem do sistema do restaurante ao registrar o pedido.</p>
              </section>

              {errorMessage && (
                <div className="flex items-start gap-2.5 rounded-xl border border-rose-500/20 bg-rose-500/10 p-3 text-xs font-bold text-rose-400" role="alert">
        ...
        {!createdOrder && (
          <footer className="shrink-0 border-t border-koma-border bg-koma-panel p-4 sm:px-6 sm:py-5">
            <button
              ...
              className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-4 text-xs font-black uppercase tracking-wider text-white transition hover:bg-emerald-600 disabled:cursor-wait disabled:opacity-55"
