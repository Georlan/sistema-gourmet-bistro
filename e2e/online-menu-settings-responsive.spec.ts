...
for (const theme of ['dark', 'light'] as const) {
  for (const viewport of mobileViewports) {
    test(`cardápio online mantém hierarquia enxuta em ${viewport.width}x${viewport.height} no tema ${theme}`, async ({ page }) => {
      ...
      await expect(page.locator('.cashier-subnav')).toBeHidden();
      ...
      await expect(page.getByLabel('Valor por km da entrega')).toBeVisible();
      await expectNoHorizontalOverflow(page);

      await navigate(page, 'Pagamentos');
      ...
      await navigate(page, 'Divulgação');
