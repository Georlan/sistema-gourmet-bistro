...
test.describe('checkout público de adesão KÔMA', () => {
  test('seleciona plano e ciclo, sincroniza a URL e expõe somente pagamentos suportados', async ({ page }) => {
    ...
    const proPlan = planGroup.getByRole('radio').filter({ hasText: 'Pro' });
    const premiumPlan = planGroup.getByRole('radio').filter({ hasText: 'Premium' });
    ...
    const annualBilling = billingGroup.getByRole('radio').filter({ hasText: 'Anual' });
    ...
    await expect(paymentGroup.getByRole('radio').filter({ hasText: 'Cartão de crédito' })).toBeVisible();
    const pix = paymentGroup.getByRole('radio').filter({ hasText: 'Pix anual à vista' });
  ...
  test('mensal não oferece Pix e preserva o trial de cartão sem mensalidade fixa', async ({ page }) => {
    ...
    const planGroup = page.getByRole('radiogroup', { name: 'Escolha um plano KÔMA' });
    await expect(planGroup.getByRole('radio').filter({ hasText: 'Pocket' })).toHaveAttribute('aria-checked', 'true');
