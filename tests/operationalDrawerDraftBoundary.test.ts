...
test('operational drawer destroys the tenant-less legacy draft key before rendering', () => {
  ...
  const renderIndex = wrapper.indexOf('<OperationalDrawerLegacy');
  assert.ok(purgeIndex >= 0, 'drawer boundary must purge the legacy draft key');
  assert.ok(renderIndex > purgeIndex, 'legacy key must be purged before the drawer implementation renders');
});
