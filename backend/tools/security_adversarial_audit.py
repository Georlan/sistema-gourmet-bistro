...
def finding(code: str, severity: str, title: str, evidence: str) -> None:
    findings.append(
...
def seed_five_tenants() -> dict[int, int]:
    ...
    try:
        for rid in TENANT_IDS:
            ...
            db.add(
                Usuario(
                    id=f"audit-user-{rid}",
                    restaurante_id=rid,
                    nome=f"Audit Operator {rid}",
                    email=f"operator-{rid}@koma.test",
                    usuario=f"operator-{rid}",
        ...
        db.add(
            Usuario(
                id="audit-shared-5101",
                restaurante_id=5101,
                nome="Shared Tenant 5101",
                email=SHARED_EMAIL,
                senha_hash=SHARED_HASH_A,
                role="garcom",
                cargo="garcom",
                status="ativo",
            )
        ...
        db.add(
            Usuario(
                id="audit-shared-5102",
                restaurante_id=5102,
                nome="Shared Tenant 5102",
                email=SHARED_EMAIL,
                senha_hash=SHARED_HASH_B,
                role="garcom",
                cargo="garcom",
                status="ativo",
            )
        ...
        for rid in TENANT_IDS:
            ...
            turno = CaixaTurno(
                restaurante_id=rid,
                aberto_por_id=f"audit-user-{rid}",
                saldo_inicial=0,
                status="aberto",
            )
            db.add(turno)
            ...
            for mesa_id in range(1, OPEN_COMMANDAS_PER_TENANT + 1):
                db.add(
                    Comanda(
                        id=f"audit-t{rid}-cmd-{mesa_id:02d}",
                        restaurante_id=rid,
                        mesa_id=mesa_id,
                        garcom_id=f"audit-user-{rid}",
                        tipo="Consumo no Local",
                        numero_pedido=(rid * 100) + mesa_id,
                        valor_pago=0,
                        fechada=False,
                    )
                )
            ...
            intent = SmartPosPaymentIntent(
                id=f"audit-intent-{rid}",
                restaurante_id=rid,
                turno_id=turno.id,
                mesa_id=1,
                operador_id=f"audit-user-{rid}",
                valor=10,
                metodo="credito",
                captura="provider_integrado",
                escopo="valor",
                idempotency_key=f"audit-idempotency-{rid}",
                status="pendente",
                origem="smartpos",
            )
            db.add(intent)
            ...
            db.add(
                SmartPosPaymentIntentEvent(
                    id=f"audit-event-{rid}",
                    restaurante_id=rid,
                    intent_id=intent.id,
                    from_status="criada",
                    to_status="pendente",
                    actor_id=f"audit-user-{rid}",
                    transition_key=f"audit-transition-{rid}",
                    motivo="security audit seed",
                )
            )
        db.commit()
    finally:
...
def _tenant_array(conn, table: str) -> list[int]:
    rows = conn.execute(
        text(f"SELECT DISTINCT restaurante_id FROM public.{table} ORDER BY restaurante_id")
    ).scalars().all()
    ...
    with runtime_engine.connect() as conn:
        for rid in TENANT_IDS:
            ...
            try:
                ...
                if event_seen != [rid]:
                    finding(
                        "SMARTPOS_EVENT_RLS_MISSING",
                        "CRITICAL",
                        "SmartPOS payment event history crosses tenant boundaries at DB role level",
                        f"tenant {rid} saw event restaurante_id values {event_seen}",
                    )
            finally:
                tx.rollback()
        ...
        try:
            ...
            for table in protected:
                ...
                assert count == 0, f"fail-closed missing tenant failed on {table}: {count}"
        finally:
            tx.rollback()
        ...
        try:
            ...
            changed = conn.execute(
                text("UPDATE usuarios SET nome = nome WHERE restaurante_id = 5102")
            ).rowcount
        ...
        try:
            ...
            try:
                conn.execute(
                    text(
                        "INSERT INTO usuarios (id,nome,email,senha_hash,role,restaurante_id,cargo,status) "
                        "VALUES ('audit-illegal-user','Illegal','illegal@koma.test','x','garcom',5102,'garcom','ativo')"
                    )
                )
            except DBAPIError:
                nested.rollback()
        ...
        try:
            ...
            try:
                conn.execute(
                    text(
                        "INSERT INTO activity_logs (restaurante_id,garcom_id,action,details) "
                        "VALUES (5101,'audit-user-5102','SECURITY_AUDIT','cross tenant FK probe')"
                    )
                )
                cross_activity = True
            except DBAPIError:
            ...
            if cross_activity:
                finding(
                    ...
                    "HIGH",
                    "Child row can reference a user from another tenant through a non-composite FK",
                    "activity_logs(restaurante_id=5101, garcom_id=user from 5102) was accepted by PostgreSQL",
                )
                ...
                conn.execute(
                    text(
                        ...
                        INSERT INTO smartpos_payment_intents (
                            id,restaurante_id,turno_id,mesa_id,operador_id,valor,metodo,captura,
                            escopo,idempotency_key,status,status_em,origem,criado_em
                        ) VALUES (
                            'audit-cross-intent',5101,:turno,1,'audit-user-5102',1.00,'credito',
                            'provider_integrado','valor','audit-cross-intent-key','pendente',now(),'smartpos',now()
                        )
                        """
                    ),
                    {"turno": turno_ids[5101]},
                )
                cross_intent = True
            except DBAPIError:
                pass
            ...
            if cross_intent:
                finding(
                    ...
                    "HIGH",
                    "SmartPOS intent can reference an operator from another tenant through raw SQL",
                    "intent tenant 5101 with operador_id from tenant 5102 passed FK validation",
                )
        finally:
            tx.rollback()
        ...
        try:
            ...
            if before == [5101] and after == [5102]:
                finding(
                    "RLS_GUC_MUTABLE_BY_RUNTIME",
                    "MEDIUM",
                    "Tenant GUC is mutable by the runtime SQL role",
                    "a connection with arbitrary SQL execution can change app.current_restaurante_id from 5101 to 5102",
                )
        finally:
            tx.rollback()
...
async def audit_http_and_busy_day() -> None:
    tokens = {
        rid: create_access_token(
            subject=f"audit-user-{rid}", restaurante_id=rid, role="garcom"
        )
        for rid in TENANT_IDS
    }
    ...
    async with httpx.AsyncClient(
        transport=transport,
        base_url="http://koma-security.test",
        timeout=30.0,
    ) as client:
        async def check_one(rid: int, index: int) -> float:
            path = (
                ...
                if index % 3 == 1
                else "/comandas/detalhes/todos?fechada=false"
            )
            started = time.perf_counter()
            response = await client.get(
                path, headers={"Authorization": f"Bearer {tokens[rid]}"}
            )
            elapsed = (time.perf_counter() - started) * 1000
            assert response.status_code == 200, (rid, path, response.status_code, response.text[:300])
            data = response.json()
        ...
        durations = await asyncio.gather(
            *[
                check_one(rid, index)
                for rid in TENANT_IDS
                for index in range(BUSY_REQUESTS_PER_TENANT)
            ]
        )
        ordered = sorted(durations)
        metrics["busy_day"] = {
            "restaurants": len(TENANT_IDS),
            "open_comandas": len(TENANT_IDS) * OPEN_COMMANDAS_PER_TENANT,
            "http_requests": len(durations),
            "latency_ms_p50": round(statistics.median(ordered), 2),
            "latency_ms_p95": round(ordered[int(len(ordered) * 0.95) - 1], 2),
            "latency_ms_max": round(max(ordered), 2),
        }

        mismatch = create_access_token(
            subject="audit-user-5101", restaurante_id=5102, role="garcom"
        )
        response = await client.get(
            "/auth/smartpos/contexto",
        ...
        expired = jwt.encode(
            {"sub": "audit-user-5101", "restaurante_id": 5101, "exp": 1},
            settings.SECRET_KEY,
        ...
        malformed_tenant = await client.post(
            "/auth/login",
            json={
                "username": "operator-5101@koma.test",
                "password": PASSWORD,
                "restaurante_id": "5101 OR 1=1",
            },
        )
        assert malformed_tenant.status_code == 422

        shared_a = await client.post(
            "/auth/login",
            json={"username": SHARED_EMAIL, "password": "Shared-A-123"},
        )
        shared_b = await client.post(
            "/auth/login",
            json={"username": SHARED_EMAIL, "password": "Shared-B-456"},
        )
        assert shared_a.status_code == 200, shared_a.text
        assert shared_b.status_code == 200, shared_b.text
        assert shared_a.json()["usuario"]["restaurante_id"] == 5101
        assert shared_b.json()["usuario"]["restaurante_id"] == 5102
        ...
        for _ in range(20):
            ...
            bad_login_statuses.append(attempt.status_code)
        if 429 not in bad_login_statuses:
            finding(
                "STAFF_LOGIN_NO_RATE_LIMIT",
                "HIGH",
                "Staff login accepts repeated password attempts without throttling",
                f"20 consecutive bad attempts returned statuses {sorted(set(bad_login_statuses))}; no HTTP 429",
            )
