...
def test_02_pedido_misto_gera_producao_apenas_com_itens_cozinha():
    ...
    assert "2 x 001 - HAMBURGUER TRADICIONAL" in doc_text
    assert "R$ 50,00" in doc_text
    assert "COCA-COLA" not in doc_text
    assert "TOTAL GERAL DA MESA:" not in doc_text
...
def test_03_itens_de_clientes_diferentes_nao_sao_agrupados_juntos():
    ...
    assert "SUBTOTAL" not in doc_text
    assert "TOTAL GERAL DA MESA:" not in doc_text
...
def test_07_producao_local_contem_valores_sem_total_de_fechamento():
    order = OrderPrintData(
        numero_pedido="107",
        tipo_pedido="Consumo no Local",
        mesa="5",
    ...
    assert "2 x 001 - CHEESEBURGER" in doc_text
    assert "R$ 71,80" in doc_text
    assert "TOTAL GERAL DA MESA:" not in doc_text
    assert "Gerenciado por Kôma" not in doc_text
    assert "Documento não fiscal" not in doc_text
